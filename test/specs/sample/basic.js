...
test('Check app config', (assert) => {
  ...
  assert.ok(bella.isObject(config.pusher), 'config.pusher must be an object');
  assert.end();
});
...
test('Check default page', (assert) => {
  request(target)
    ...
    .expect((res) => {
      ...
      assert.ok(txt.startsWith('BodyTrace checker'), 'Text must be started with a correct title');
    }).end(assert.end);
});
